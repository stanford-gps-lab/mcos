% Loop through different PER and plot results
% Andrew Neish - 02/6/2019
clear all
close all
clc
...
run_param.OTAR_num = 2000;  % Number of OTAR messages to be generated
run_param.user_loop = 1;   % Number of times a user is simulated for each 'Start Time'
run_param.plots = {};
w12 = 135.3048;         % Set weight ratio of OMT1 and OMT2 to the rest of the OMTs



% Set PER values to loop through
run_param.PER_loop = linspace(1e-1, 1e-5, 100)';
...
% Initialize waitbar
L_waitbar = waitbar(0, ['Looping Through PER... Sim Length: ', num2str(run_param.OTAR_num)]);    % Initialize waitbar

if strcmp(run_param.scheme, 'ECDSA')
    data = NaN(length(run_param.PER_loop), 6);
elseif strcmp(run_param.scheme, 'TESLA')
    data = NaN(length(run_param.PER_loop), 9);
end
...
for i = 1:length(run_param.PER_loop)
    ...
    clc
    disp(['Simulating user with PER (', num2str(i), '/', num2str(length(run_param.PER_loop)), '): '...
        num2str(length(run_param.PER_loop(i)))])
    disp(['Current Simulation Length: ', num2str(run_param.OTAR_num)])
    waitbar(i/length(run_param.PER_loop), L_waitbar, ['Looping Through PER ', '(', num2str(i), '/', num2str(length(run_param.PER_loop)), ')', newline,...
...
if strcmp(run_param.scheme, 'ECDSA')     
    figure
    loglog(run_param.PER_loop, data(:,1), 'g', 'LineWidth', 0.01)
    hold on
    h1 = loglog(run_param.PER_loop, data(:,2), 'g--', 'LineWidth', 2);
    loglog(run_param.PER_loop, data(:,3), 'g', 'LineWidth', 0.01)
    loglog(run_param.PER_loop, data(:,4),'b', 'LineWidth', 0.01)
    h2 = loglog(run_param.PER_loop, data(:,5),'b--', 'LineWidth', 2);
    loglog(run_param.PER_loop, data(:,6),'b', 'LineWidth', 0.01)
    temp1 = [run_param.PER_loop', fliplr(run_param.PER_loop')];
    ...
    if run_param.num_GUS_sites == 1
        title('Impact of PER on message reception - Case 1')
    elseif run_param.num_GUS_sites > 1
        title('Impact of PER on message reception - Case 2')
    end
    xlabel('Page Error Rate')
...
elseif strcmp(run_param.scheme, 'TESLA')     
    ...
    h1 = loglog(run_param.PER_loop, data(:,2), 'g-.', 'LineWidth', 2);
    loglog(run_param.PER_loop, data(:,3), 'g', 'LineWidth', 0.01)
    loglog(run_param.PER_loop, data(:,4),'r', 'LineWidth', 0.01)
    h2 = loglog(run_param.PER_loop, data(:,5),'r-.', 'LineWidth', 2);
    loglog(run_param.PER_loop, data(:,6),'r', 'LineWidth', 0.01)
    loglog(run_param.PER_loop, data(:,7),'b', 'LineWidth', 0.01)
    h3 = loglog(run_param.PER_loop, data(:,8),'b-.', 'LineWidth', 2);
    loglog(run_param.PER_loop, data(:,9),'b', 'LineWidth', 0.01)
    temp1 = [run_param.PER_loop', fliplr(run_param.PER_loop')];
    temp2 = [data(:,1)', fliplr(data(:,3)')];
    h = fill(temp1, temp2, 'g');
    set(h,'facealpha',.2,'edgealpha',0)
    ...
    ylim([30 inf])
    if run_param.num_GUS_sites == 1
        title('Impact of PER on message reception - Case 3')
    elseif run_param.num_GUS_sites > 1
        title('Impact of PER on message reception - Case 4')
    end
    xlabel('Page Error Rate')
    ylabel('Time to receive set of messages')
...
if run_param.save_data
    ...
    str = [run_param.scheme, '_',...
        ...
        'PER_min', num2str(run_param.PER_loop(1),'%.E'), '_',...
        'PER_max', num2str(run_param.PER_loop(end),'%.E'), '_',...
        'PER_num', num2str(length(run_param.PER_loop),'%.E'),...
        ];
    
    % Save to data directory
    try
        cd([pwd, '/Data/PER_Loops/', str])
    catch
        mkdir([pwd, '/Data/PER_Loops/', str])
        cd([pwd, '/Data/PER_Loops/', str])
    end
    save(str)
